# Run Codex in production without Docker
...
Here you can find the scripts and known working process to run Codex outside of a Docker container.
...
`git clone git@github.com:Dimatic-Labs/codex.git`

2. `cd codex` and run `yarn setup`. This will install all dependencies to run in production as well as debug the application.
...
Codex is comprised of three main sections. The `frontend`, `server`, and `collector`. When running in production you will be running `server` and `collector` on two different processes, with a build step for compilation of the frontend.
...
Codex should now be running on `http://localhost:3001`!

## Updating Codex

To update Codex with future updates you can `git pull origin master` to pull in the latest code and then repeat steps 2 - 5 to deploy with all changes fully.
...
_note_ You should `pkill node` before running an update so that you are not running multiple Codex processes on the same instance as this can cause conflicts.
...
cd $HOME/codex/frontend && yarn && yarn build && cd $HOME/codex
...
cd $HOME/codex/collector && yarn

echo "Installing server dependencies & running migrations"
cd $HOME/codex/server && yarn
cd $HOME/codex/server && npx prisma migrate deploy --schema=./prisma/schema.prisma
cd $HOME/codex/server && npx prisma generate

echo "Booting up services."
truncate -s 0 /logs/server.log # Or any other log file location.
truncate -s 0 /logs/collector.log

cd $HOME/codex/server
(NODE_ENV=production node index.js) &> /logs/server.log &

cd $HOME/codex/collector
