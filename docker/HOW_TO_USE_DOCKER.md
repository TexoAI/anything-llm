# How to use Dockerized Codex

Use the Dockerized version of Codex for a much faster and complete startup of Codex.

### Minimum Requirements

> [!TIP]
> Running Codex on AWS/GCP/Azure?
...
\*Codex by default uses a built-in vector database powered by [LanceDB](https://github.com/lancedb/lancedb)

\*Codex by default embeds text on instance privately [Learn More](../server/storage/models/README.md)

## Recommend way to run dockerized Codex!
...
> the docker container using Codex as `localhost:xxxx` will not resolve for the host system.
>
...
<table>
<tr>
<th colspan="2">Mount the storage locally and run Codex in Docker</th>
</tr>
<tr>
<td>
  Linux/MacOs
...
```shell
export STORAGE_LOCATION=$HOME/codex && \
...
$env:STORAGE_LOCATION="$HOME\Documents\codex"; `
...
services:
  codex:
    ...
    volumes:
      - codex_storage:/app/server/storage
    restart: always
...
Go to `http://localhost:3001` and you are now using Codex! All your data and progress will persist between
...
- `git clone` this repo and `cd codex` to get to the root directory.
- `touch server/storage/codex.db` to create empty SQLite DB file.
- `cd docker/`
...
The integrations below are templates or tooling built by the community to make running the docker experience of Codex easier.

### Use the Midori AI Subsystem to Manage Codex

Follow the setup found on [Midori AI Subsystem Site](https://io.midori-ai.xyz/subsystem/manager/) for your host OS
After setting that up install the Codex docker backend to the Midori AI Subsystem.
...
Then in docker you need to replace that localhost part with `host.docker.internal`. For example, if running Ollama on the host machine, bound to http://127.0.0.1:11434 you should put `http://host.docker.internal:11434` into the connection URL in Codex.
