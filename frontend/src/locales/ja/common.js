...
const TRANSLATIONS = {
  onboarding: {
    ...
    llm: {
      title: "LLMの設定",
      description:
        "Codexは多くのLLMプロバイダーと連携できます。これがチャットを処理するサービスになります。",
    },
    userSetup: {
      title: "ユーザー設定",
    ...
    survey: {
      title: "Codexへようこそ",
      description:
        "Codexをあなたのニーズに合わせて構築するためにご協力ください。任意です。",
      email: "メールアドレスは何ですか？",
      useCase: "Codexを何に使用しますか？",
      useCaseWork: "仕事用",
      useCasePersonal: "個人用",
      useCaseOther: "その他",
      comment: "Codexをどのように知りましたか？",
      commentPlaceholder:
  ...
  welcomeMessage: {
    part1:
      "Codexへようこそ。CodexはDimaticによるオープンソースのAIツールで、あらゆるものをトレーニングされたチャットボットに変換し、クエリやチャットが可能です。CodexはBYOK（Bring-Your-Own-Keys）ソフトウェアであり、このソフトウェアを使用するためのサブスクリプション、料金、または費用はありません。",
    part2:
      "Codexは、OpenAi、GPT-4、LangChain、PineconeDB、ChromaDBなどの強力なAI製品を簡単に統合し、手間をかけずに生産性を100倍に向上させる最も簡単な方法です。",
    part3:
      "Codexは、あなたのマシン上で完全にローカルで動作し、ほとんど負荷をかけません。GPUは必要ありません。クラウドおよびオンプレミスでのインストールも可能です。\nAIツールエコシステムは日々強力になっています。Codexはそれを簡単に利用できるようにします。",
  ...
  api: {
    title: "APIキー",
    description:
      "APIキーにより、プログラム経由でこのCodexインスタンスにアクセスおよび管理できます。",
    link: "APIドキュメントを読む",
  ...
  llm: {
    title: "LLMの設定",
    description:
      "これは、お好みのLLMチャットおよび埋め込みプロバイダー用の認証情報と設定です。これらのキーが最新かつ正確でない場合、Codexは正しく動作しません。",
    provider: "LLMプロバイダー",
    providers: {
      azure_openai: {
  ...
  transcription: {
    ...
    "warn-start":
      "RAMやCPUが限られたマシンでローカルのWhisperモデルを使用すると、メディアファイルの処理中にCodexが停止する可能性があります。",
    "warn-recommend":
  ...
  embedding: {
    ...
    "desc-end":
      "埋め込みとは、テキストをベクトルに変換するプロセスです。これらの認証情報は、ファイルやプロンプトをCodexが処理できるフォーマットに変換するために必要です。",
    provider: {
      title: "埋め込みプロバイダー",
    },
  },
  ...
  vector: {
    title: "ベクターデータベース設定",
    description:
      "これは、Codexインスタンスの動作方法用の認証情報と設定です。これらのキーが最新で正確であることが重要です。",
    provider: {
  ...
  privacy: {
    title: "プライバシーとデータ処理",
    description:
      "これは、接続されているサードパーティプロバイダーとCodexがデータをどのように処理するかの設定です。",
    llm: "LLM選択",
    embedding: "埋め込み設定",
    vector: "ベクターデータベース",
  ...
  connectors: {
    ...
    upload: {
      ...
      "privacy-notice":
        "これらのファイルは、このCodexインスタンス上のドキュメント処理機能にアップロードされます。第三者に送信・共有されることはありません。",
    },
    pinning: {
      what_pinning: "ドキュメントのピン留めとは？",
      pin_explained_block1:
        "Codexでドキュメントを<b>ピン留め</b>すると、その内容全体がプロンプトウィンドウに挿入され、LLMがしっかり理解できるようになります。",
    ...
    watching: {
      ...
      watch_explained_block1:
        "Codexでドキュメントを<b>ウォッチ</b>すると、元のソースから定期的に内容が<i>自動的に</i>同期されます。管理しているすべてのワークスペースで内容が自動更新されます。",
  ...
  security: {
    ...
    password: {
      title: "パスワード保護",
      description:
        "Codexインスタンスをパスワードで保護します。これを忘れた場合、回復方法はないため、このパスワードを必ず保存してください。",
