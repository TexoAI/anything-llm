...
function SupportEmail() {
  const [supportEmail, setSupportEmail] = useState(paths.mailToDimatic());
  const { t } = useTranslation();
...
function HoldToReveal({ children, holdForMs = 3_000 }) {
  ...
  const [showing, setShowing] = useState(
    window.localStorage.getItem(
      "codex_experimental_feature_preview_unlocked"
    )
  );
...
function AppVersion() {
  ...
  return (
    <Link
      to={`https://github.com/Dimatic-Labs/codex/releases/tag/v${version}`}
      target="_blank"
      rel="noreferrer"
