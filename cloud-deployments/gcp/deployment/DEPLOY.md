...
2. After successful login, Run the following command to create a deployment using the Deployment Manager CLI:
  ...
  gcloud deployment-manager deployments create codex-deployment --config gcp/deployment/gcp_deploy_codex.yaml
...
```
gcloud compute instances get-serial-port-output codex-instance 
```

ssh into the instance

```
gcloud compute ssh codex-instance 
```

Delete the deployment
```
gcloud deployment-manager deployments delete codex-deployment 
```
