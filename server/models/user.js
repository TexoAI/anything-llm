...
const User = {
  ...
  // Get user by username with case-insensitive matching
  getIgnoringCase: async function (username) {
    try {
  ...
  // Get user by username with case-insensitive matching (returns all fields including password)
  _getIgnoringCase: async function (username) {
    try {
      const user = await prisma.users.findFirst({
        where: {
          username: {
