...
const client = new OpenAI({
  ...
  apiKey: "ENTER_CODEX_API_KEY_HERE",
});
...
(async () => {
  ...
  const syncCompletion = await client.chat.completions.create({
    messages: [
      ...
      // {
      //   role: 'assistant',
      //   content: "Arrr, matey! Codex be a fine tool fer sailin' the treacherous sea o' information with a powerful language model at yer helm. It's a potent instrument to handle all manner o' tasks involvin' text, like answerin' questions, generating prose, or even havin' a chat with digital scallywags like meself. Be there any specific treasure ye seek in the realm o' Codex?"
      // },
      // { role: "user", content: "Why are you talking like a pirate?" },
    ],
    model: "codex", // must be workspace-slug
  });
  ...
  const embedding = await client.embeddings.create({
    model: null, // model is optional for Codex
