...
const PGVector = {
   ...
   * - Defaults to "codex_vectors" if no table name is provided.
   * @returns {string}
   */
  tableName: () => process.env.PGVECTOR_TABLE_NAME || "codex_vectors",
  ...
  validateExistingEmbeddingTableSchema: async function (pgClient, tableName) {
    ...
    if (result.rows.length === 0)
      throw new Error(
        `The table '${tableName}' was found but does not contain any columns or cannot be accessed by role. It cannot be used as an embedding table in Codex.`
      );
   ...
   * so that codex can manage the table directly.
   *
  ...
  /**
   * Check if the codex embedding table exists in the database
   * @returns {Promise<boolean>}
   */
  ...
  /**
   * Reset the entire vector database table associated with codex
